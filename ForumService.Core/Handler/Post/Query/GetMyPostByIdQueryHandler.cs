...
{
    /// <summary>
    /// Handles retrieving a single post owned by the requester, regardless of its status.
    /// </summary>
    public class GetMyPostByIdQueryHandler : IQueryHandler<GetMyPostByIdQuery, BaseResponseDto<PostViewDetailDto>>
    {
        ...
        public GetMyPostByIdQueryHandler(
            ...
            ILogger<GetMyPostByIdQueryHandler> logger)
        {
        ...
        public async Task<BaseResponseDto<PostViewDetailDto>> Handle(GetMyPostByIdQuery request, CancellationToken cancellationToken)
        {
            try
            {
                // *** KEY CHANGE ***
                // Fetches the post ONLY if the PostId matches, RequesterId matches the AuthorId, and it's not deleted.
                // It IGNORES the 'Status' (so Drafts, Pending, etc. can be fetched).
                var postEntity = await _postRepository.GetOneAsync(
                    filter: p => p.PostId == request.PostId && p.AuthorId == request.RequesterId && !p.IsDeleted
                );
                ...
                if (postEntity == null)
                {
                    ...
                    {
                        Status = 404,
                        Message = $"Post with ID {request.PostId} not found or you do not have permission to view it.",
                        ResponseData = null
                    };
                }

                // --- Logic is identical to GetPostDetailsByIdQueryHandler from here ---
