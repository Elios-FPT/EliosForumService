...
{
    public class GetModeratorPublicPostsQueryHandlerTests
    {
        // Mocks
        ...
        // System Under Test
        private readonly GetModeratorPublicPostsQueryHandler _handler;

        public GetModeratorPublicPostsQueryHandlerTests()
        {
            _mockPostQueryRepository = new Mock<IPostQueryRepository>();
            _mockProducerRepository = new Mock<IKafkaProducerRepository<User>>();

            _handler = new GetModeratorPublicPostsQueryHandler(
        ...
        [Fact]
        public async Task Handle_ShouldReturnEmptyList_WhenNoPublishedPostsFound()
        {
            // Arrange
            var query = new GetModeratorPublicPostsQuery();

            // Mock Repo to return empty list
            _mockPostQueryRepository.Setup(x => x.GetModeratorPublicViewPostsAsync(query))
            ...
            Assert.Equal("No published posts found.", result.Message);
        ...
        public async Task Handle_ShouldReturnMappedPosts_WhenPostsAndUsersExist()
        {
            ...
            {
                new User { id = authorId, firstName = "John", lastName = "Author", avatarUrl = "auth.png" },
                new User { id = moderatorId, firstName = "Jane", lastName = "Mod", avatarUrl = "mod.png" }
            };
            ...
            Assert.Equal("Technology", dto.CategoryName);
            Assert.Equal("Published", dto.Status);
            Assert.Equal("John", dto.AuthorFirstName);
            Assert.Equal("Jane", dto.ModeratorFirstName);
        }
        ...
        public async Task Handle_ShouldReturnPostsWithoutUserDetails_WhenUserServiceFails()
        {
            ...
            var posts = new List<Domain.Models.Post>
            {
                new Domain.Models.Post { PostId = Guid.NewGuid(), Title = "Safe Post", AuthorId = Guid.NewGuid() }
            };

            _mockPostQueryRepository.Setup(x => x.GetModeratorPublicViewPostsAsync(query))
                .ReturnsAsync((posts, 1));

            // Mock Kafka to throw exception (e.g. timeout)
            _mockProducerRepository.Setup(x => x.ProduceGetAllAsync(
                    It.IsAny<string>(),
                    It.IsAny<string>(),
                    It.IsAny<string>(),
                    It.IsAny<CancellationToken>()))
                .ThrowsAsync(new Exception("Kafka Service Unreachable"));
            ...
            Assert.NotNull(result);
            Assert.Single(result.ResponseData);
            Assert.Equal("Moderator public posts retrieved successfully.", result.Message); // Success despite kafka failure

            var dto = result.ResponseData.First();
            Assert.Equal("Safe Post", dto.Title);
        ...
        {
            // Arrange
            var query = new GetModeratorPublicPostsQuery(Page: 0, Size: -5); // Invalid pagination
        ...
        public async Task Handle_ShouldReturn500_WhenRepositoryThrowsException()
        {
            ...
            _mockPostQueryRepository.Setup(x => x.GetModeratorPublicViewPostsAsync(query))
                .ThrowsAsync(new Exception("Database Critical Failure"));
            ...
            Assert.Contains("Database Critical Failure", result.Message);
        }
    }
}
